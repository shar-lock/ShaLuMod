...
/// 且整牌只耗一次、增益作用于两段（旧版为每段各耗一次——两层血仇换两次加成，过于吃亏）。
/// </summary>
public class RepeatedThrust : WandiModCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        for (int i = 0; i < 2; i++)
            await DamageCmd.Attack(hit).FromCard(this, cardPlay).Targeting(cardPlay.Target).WithValueProp(ValueProp.Move).Execute(choiceContext);
    }
}
