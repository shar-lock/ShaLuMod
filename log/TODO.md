...
## 08/03 游戏测试 Bug 修复（本批）

### 已修（代码完成，待局内回归）

| # | 问题 | 处理 |
|---|---|---|
| 1 | 消耗血仇也扣荡平进度 | `ConquerProgressPower` 只累加血仇**获得量**并负责生成荡平万邦；`VengeancePower` 仅成长消耗 4，不再 SyncProgress / 生成卡 |
| 2 | 局外「灾厄之矛」「命途斩」等显示造成 0 伤 | 描述改为组成文案 + `{InCombat:（造成{CalculatedDamage}点伤害）\|}`（BodySlam 范式）；同改万死无悔/噬仇/荡平/焚天/诛王枪/王座/此乃天谴 |
| 3 | 狂战之心每攻击自伤 | 去掉自伤，仅 +1 血仇 |
| 4 | 血祭仪典数值/词条 | 失 3 抽 3 +3 血仇 / 抽 5；加 Exhaust |
| 5 | 遗物稀有度 | 焚血勋章 Rare；血仇圣坛 Uncommon |
| 6 | 战意持久不叠加未标注 | 卡面「效果不叠加」；Power 仍 `StackType.Single` |
| 7 | 灼血击条件满足无黄框 | `ShouldGlowGoldInternal`：`DisplayAmount > 4` |

### 待游戏内回归（本批）
- 荡平进度：打牌消耗血仇**不减**进度；累计获得满 7 → 入手荡平万邦（不扣血仇）
- 血仇自身：显示积到 7 仍消耗最多 4 层
- 局外/选牌库查看灾厄之矛·命途斩：只见「最大生命 X%」；战斗中手牌另见括号总伤
- 狂战之心：打攻击只 +1 血仇、不掉血
- 血祭仪典：消耗词条 + 失3/抽3/+3 / 升级抽5
- 掉落池：焚血勋章稀有、血仇圣坛罕见
- 战意持久打第二次：减伤不叠更高；文案有「不叠加」
- 灼血击：显示血仇>4 时金边

---

## 08/03 远程拉取审查（69931ae…99429dd）

相对 `f44c56c` 新增 2 commit：`69931ae`（平衡微调）→ `99429dd`（建筑师对话 + 攻击 VFX）。

### 审查已修
| # | 问题 | 处理 |
|---|---|---|
| 1 | 沾血枪尖用 `Random.Shared` 选目标 → 联机可能不同步 | 改 `Owner.RunState.Rng.CombatTargets.NextItem`（Tingsha 范式） |
| 2 | eng `card_keywords` 荡平仍写 Retain（zhs 已删） | 去掉 Retain，对齐代码仅 Exhaust |

### 待游戏内回归（本批）
- 沾血枪尖：获血仇 → **随机单体 3 伤**（非全体 1）
- 金焰斩 12/15；弑王连突 10/14；狂怒 ×2/3；坚壁升级回血 10
- 建筑师：首次通关 3 行对话；重复通关 2 行；终局斩击/血溅 VFX
- 回合结束 ping 文案（zhs banter）

### 备注
- 建筑师 Patch 对 `DefineDialogues` Postfix + loc key `WANDIMOD-WANDI_MOD` 结构与原版一致；`GetArchitectAttackVfx` override 合理
- 遗物设计稿「沾血枪尖」段落下说明仍写「卖血折现成血仇」（旧机制口吻），正文效果表已更新——文档小瑕疵
...
- 灾厄之矛 / 命途斩 / 此乃天谴：战斗内 `{CalculatedDamage}` 括号与实打一致（局外见组成文案）
