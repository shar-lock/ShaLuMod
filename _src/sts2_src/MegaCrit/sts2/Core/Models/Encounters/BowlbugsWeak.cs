...
// Type: MegaCrit.Sts2.Core.Models.Encounters.BowlbugsWeak
...
public sealed class BowlbugsWeak : EncounterModel
{
  ...
  public override IEnumerable<MonsterModel> AllPossibleMonsters
  {
    get
    {
      return ((IEnumerable<MonsterModel>) BowlbugsWeak.Bugs).Concat<MonsterModel>((IEnumerable<MonsterModel>) new \u003C\u003Ez__ReadOnlySingleElementList<MonsterModel>((MonsterModel) ModelDb.Monster<BowlbugRock>()));
    }
  }
  ...
  protected override IReadOnlyList<(MonsterModel, string?)> GenerateMonsters()
  {
    ...
    return (IReadOnlyList<(MonsterModel, string)>) new \u003C\u003Ez__ReadOnlyArray<(MonsterModel, string)>(new (MonsterModel, string)[2]
    {
      ...
      (this.Rng.NextItem<MonsterModel>((IEnumerable<MonsterModel>) BowlbugsWeak.Bugs).ToMutable(), "even")
    });
  }
