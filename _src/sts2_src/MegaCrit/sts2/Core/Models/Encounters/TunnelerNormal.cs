...
// Type: MegaCrit.Sts2.Core.Models.Encounters.TunnelerNormal
...
public sealed class TunnelerNormal : EncounterModel
{
  ...
  public override IEnumerable<MonsterModel> AllPossibleMonsters
  {
    get
    {
      ...
      {
        (MonsterModel) ModelDb.Monster<Tunneler>(),
        (MonsterModel) ModelDb.Monster<Chomper>()
      });
    }
  }
  ...
  {
    Chomper mutable = (Chomper) ModelDb.Monster<Chomper>().ToMutable();
    mutable.ScreamFirst = true;
    // ISSUE: object of a compiler-generated type is created
    return (IReadOnlyList<(MonsterModel, string)>) new \u003C\u003Ez__ReadOnlyArray<(MonsterModel, string)>(new (MonsterModel, string)[2]
    {
      ((MonsterModel) mutable, (string) null),
      (ModelDb.Monster<Tunneler>().ToMutable(), (string) null)
    });
  }
}
