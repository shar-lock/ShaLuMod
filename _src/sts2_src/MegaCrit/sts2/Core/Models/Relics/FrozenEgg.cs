...
// Type: MegaCrit.Sts2.Core.Models.Relics.FrozenEgg
...
public sealed class FrozenEgg : RelicModel
{
    ...
    CardCreationOptions options)
  {
    if (player != this.Owner || options.Flags.HasFlag((Enum) CardCreationFlags.NoHookUpgrades))
      return false;
    EggRelicHelper.UpgradeValidCards(cardRewards, CardType.Power, (RelicModel) this);
    return true;
  }
    ...
    List<CardCreationResult> cards)
  {
    if (player != this.Owner)
      return;
    EggRelicHelper.UpgradeValidCards(cards, CardType.Power, (RelicModel) this);
  }

  public override bool TryModifyCardBeingAddedToDeck(CardModel card, out CardModel? newCard)
  {
    newCard = (CardModel) null;
    if (card.Owner != this.Owner || card.Type != CardType.Power || !card.IsUpgradable)
      return false;
