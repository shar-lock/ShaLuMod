...
// Type: MegaCrit.Sts2.Core.Models.Relics.SealOfGold
...
public sealed class SealOfGold : RelicModel
{
  ...
  {
    if (!participants.Contains<Creature>(this.Owner.Creature) || this.Owner.Gold < this.DynamicVars.Gold.IntValue)
      return;
    this.Flash();
    await PlayerCmd.GainEnergy(this.DynamicVars.Energy.BaseValue, this.Owner);
    await PlayerCmd.LoseGold((Decimal) this.DynamicVars.Gold.IntValue, this.Owner);
  }
}
