...
// Type: MegaCrit.Sts2.Core.Models.Relics.MoltenEgg
...
public sealed class MoltenEgg : RelicModel
{
    ...
    CardCreationOptions options)
  {
    if (player != this.Owner || options.Flags.HasFlag((Enum) CardCreationFlags.NoHookUpgrades))
      return false;
    EggRelicHelper.UpgradeValidCards(cardRewards, CardType.Attack, (RelicModel) this);
    return true;
  }
    ...
    List<CardCreationResult> cards)
  {
    if (player != this.Owner)
      return;
    EggRelicHelper.UpgradeValidCards(cards, CardType.Attack, (RelicModel) this);
  }

  public override bool TryModifyCardBeingAddedToDeck(CardModel card, out CardModel? newCard)
  {
    newCard = (CardModel) null;
    if (card.Owner != this.Owner || card.Type != CardType.Attack || !card.IsUpgradable || card.CurrentUpgradeLevel >= 1)
      return false;
