...
// Type: MegaCrit.Sts2.Core.Models.Relics.InfusedCore
...
public sealed class InfusedCore : RelicModel
{
    ...
    ICombatState combatState)
  {
    if (!participants.Contains<Creature>(this.Owner.Creature) || this.Owner.PlayerCombatState.TurnNumber > 1)
      return;
    for (int i = 0; (Decimal) i < this.DynamicVars["Lightning"].BaseValue; ++i)
      await OrbCmd.Channel<LightningOrb>((PlayerChoiceContext) new BlockingPlayerChoiceContext(), this.Owner);
  }

  public override Decimal ModifyOrbValue(OrbModel orb, Decimal value)
  {
    return orb.Owner != this.Owner || !(orb is LightningOrb) ? value : value + this.DynamicVars["ExtraDamage"].BaseValue;
  }
}
