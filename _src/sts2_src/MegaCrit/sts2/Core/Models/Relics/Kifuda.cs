...
// Type: MegaCrit.Sts2.Core.Models.Relics.Kifuda
...
public sealed class Kifuda : RelicModel
{
  ...
  {
    get => HoverTipFactory.FromEnchantment<Adroit>(3);
  }
  ...
  public override async Task AfterObtained()
  {
    ...
    foreach (CardModel card in await CardSelectCmd.FromDeckForEnchantment(this.Owner, (EnchantmentModel) canonicalEnchantment, 3, prefs))
    {
      CardCmd.Enchant(canonicalEnchantment.ToMutable(), card, 3M);
      CardCmd.Preview(card);
    }
    canonicalEnchantment = (Adroit) null;
  }
}
