...
// Type: MegaCrit.Sts2.Core.Models.PotionPools.SilentPotionPool
...
public sealed class SilentPotionPool : PotionPoolModel
{
  ...
  {
    return (IEnumerable<PotionModel>) Silent4Epoch.Potions;
  }

  public override IEnumerable<PotionModel> GetUnlockedPotions(UnlockState unlockState)
  {
    return !unlockState.IsEpochRevealed<Silent4Epoch>() ? (IEnumerable<PotionModel>) Array.Empty<PotionModel>() : this.GenerateAllPotions();
  }
}
