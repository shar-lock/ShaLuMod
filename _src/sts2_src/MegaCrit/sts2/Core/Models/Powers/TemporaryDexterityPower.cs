...
// Type: MegaCrit.Sts2.Core.Models.Powers.TemporaryDexterityPower
...
public abstract class TemporaryDexterityPower : PowerModel, ITemporaryPower
{
  ...
  public PowerModel InternallyAppliedPower => (PowerModel) ModelDb.Power<DexterityPower>();
  ...
  protected override string SmartDescriptionLocKey
  {
    get
    {
      return !this.IsPositive ? "TEMPORARY_DEXTERITY_DOWN.smartDescription" : "TEMPORARY_DEXTERITY_POWER.smartDescription";
    }
  }
  ...
  {
    DexterityPower dexterityPower = await PowerCmd.Apply<DexterityPower>((PlayerChoiceContext) new ThrowingPlayerChoiceContext(), target, (Decimal) this.Sign * amount, applier, cardSource, true);
  }
    ...
    CardModel? cardSource)
  {
    if (amount == (Decimal) this.Amount || power != this)
      return;
    DexterityPower dexterityPower = await PowerCmd.Apply<DexterityPower>(choiceContext, this.Owner, (Decimal) this.Sign * amount, applier, cardSource, true);
  }
    ...
    IEnumerable<Creature> participants)
  {
    ...
    DexterityPower dexterityPower = await PowerCmd.Apply<DexterityPower>(choiceContext, this.Owner, (Decimal) (-this.Sign * this.Amount), this.Owner, (CardModel) null);
  }
}
