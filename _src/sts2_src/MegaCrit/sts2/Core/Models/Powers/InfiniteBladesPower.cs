...
// Type: MegaCrit.Sts2.Core.Models.Powers.InfiniteBladesPower
...
public sealed class InfiniteBladesPower : PowerModel
{
    ...
    ICombatState combatState)
  {
    if (player != this.Owner.Player)
      return;
    this.Flash();
    IEnumerable<CardModel> inHand = await Shiv.CreateInHand(this.Owner.Player, this.Amount, combatState);
  }
}
