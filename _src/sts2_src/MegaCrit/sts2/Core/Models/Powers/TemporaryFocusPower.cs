...
// Type: MegaCrit.Sts2.Core.Models.Powers.TemporaryFocusPower
...
public abstract class TemporaryFocusPower : PowerModel, ITemporaryPower
{
  ...
  public PowerModel InternallyAppliedPower => (PowerModel) ModelDb.Power<FocusPower>();
  ...
  protected override string SmartDescriptionLocKey
  {
    get
    {
      return !this.IsPositive ? "TEMPORARY_FOCUS_DOWN.smartDescription" : "TEMPORARY_FOCUS_POWER.smartDescription";
    }
  }
    ...
    get
    {
      ...
      switch (this.OriginModel)
      {
        ...
        case PotionModel model:
          collection = (IEnumerable<IHoverTip>) new \u003C\u003Ez__ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.FromPotion(model));
          break;
  ...
  {
    FocusPower focusPower = await PowerCmd.Apply<FocusPower>((PlayerChoiceContext) new ThrowingPlayerChoiceContext(), target, (Decimal) this.Sign * amount, applier, cardSource, true);
  }
    ...
    CardModel? cardSource)
  {
    if (amount == (Decimal) this.Amount || power != this)
      return;
    FocusPower focusPower = await PowerCmd.Apply<FocusPower>(choiceContext, this.Owner, (Decimal) this.Sign * amount, applier, cardSource, true);
  }
    ...
    IEnumerable<Creature> participants)
  {
    ...
    FocusPower focusPower = await PowerCmd.Apply<FocusPower>(choiceContext, this.Owner, (Decimal) (-this.Sign * this.Amount), this.Owner, (CardModel) null);
  }
}
