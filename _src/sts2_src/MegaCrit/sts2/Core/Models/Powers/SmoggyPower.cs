...
// Type: MegaCrit.Sts2.Core.Models.Powers.SmoggyPower
...
public sealed class SmoggyPower : PowerModel
{
  ...
  {
    if (card.Owner != this.Owner.Player || card.Affliction != null || card.Type != CardType.Skill || !CombatManager.Instance.History.CardPlaysStarted.Any<CardPlayStartedEntry>((Func<CardPlayStartedEntry, bool>) (e => e.HappenedThisTurn(this.CombatState) && e.CardPlay.Card.Type == CardType.Skill && e.CardPlay.Player == this.Owner.Player)))
      return;
    Smog smog = await CardCmd.Afflict<Smog>(card, 1M);
  }
