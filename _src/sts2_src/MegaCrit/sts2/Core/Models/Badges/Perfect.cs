...
// Type: MegaCrit.Sts2.Core.Models.Badges.Perfect
...
public class Perfect(SerializableRun run, bool won, ulong playerId) : Badge(run, won, playerId, "PERFECT", false, false)
{
    ...
    get
    {
      ...
      foreach (List<MapPointHistoryEntry> pointHistoryEntryList in this._run.MapPointHistory)
      {
        foreach (MapPointHistoryEntry pointHistoryEntry in pointHistoryEntryList)
        {
          ...
          {
            if (room.RoomType == RoomType.Boss && (this._won || room != this._run.MapPointHistory.Last<List<MapPointHistoryEntry>>().Last<MapPointHistoryEntry>().Rooms.Last<MapPointRoomHistoryEntry>()))
            {
              foreach (PlayerMapPointHistoryEntry playerStat in pointHistoryEntry.PlayerStats)
              {
                if ((long) playerStat.PlayerId == (long) this._localPlayer.NetId && playerStat.DamageTaken <= 0)
                  ++num;
              }
            }
          }
