...
// Type: MegaCrit.Sts2.Core.Models.Monsters.TheAdversaryMkTwo
...
public sealed class TheAdversaryMkTwo : MonsterModel
{
  ...
  {
    get => AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 200, 200);
  }
  ...
  private int FlameBeamDamage => 16 /*0x10*/;
  ...
  protected override MegaCrit.Sts2.Core.MonsterMoves.MonsterMoveStateMachine.MonsterMoveStateMachine GenerateMoveStateMachine()
  {
    ...
    MoveState initialState = new MoveState("BASH_MOVE", new Func<IReadOnlyList<Creature>, Task>(this.BashMove), new AbstractIntent[1]
    {
      (AbstractIntent) new SingleAttackIntent(this.BashDamage)
    });
  ...
  {
    AttackCommand attackCommand = await DamageCmd.Attack((Decimal) this.BashDamage).FromMonster((MonsterModel) this).WithAttackerAnim("Attack", 0.15f).WithAttackerFx(sfx: this.AttackSfx).WithHitFx("vfx/vfx_attack_slash").Execute((PlayerChoiceContext) null);
  }
