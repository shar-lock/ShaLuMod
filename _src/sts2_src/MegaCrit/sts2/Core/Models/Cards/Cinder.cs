...
// Type: MegaCrit.Sts2.Core.Models.Cards.Cinder
...
public sealed class Cinder : CardModel
{
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    AttackCommand attackCommand = await DamageCmd.Attack(this.DynamicVars.Damage.BaseValue).FromCard((CardModel) this, cardPlay).WithAttackerAnim(Ironclad.GetHeavyAnimIfApplicable(this.Owner.Character), Ironclad.GetHeavyAttackDelayIfApplicable(this.Owner.Character)).Targeting(cardPlay.Target).WithHitVfxNode((Func<Creature, Node2D>) (t => (Node2D) NFireBurstVfx.Create(t, 0.75f))).Execute(choiceContext);
