...
// Type: MegaCrit.Sts2.Core.Models.Cards.CompileDriver
...
public sealed class CompileDriver : CardModel
{
  ...
  protected override IEnumerable<DynamicVar> CanonicalVars
  {
    get
    {
      return (IEnumerable<DynamicVar>) new \u003C\u003Ez__ReadOnlyArray<DynamicVar>(new DynamicVar[4]
      {
        ...
        (DynamicVar) new CalculatedVar("CalculatedCards").WithMultiplier((Func<CardModel, Creature, Decimal>) ((card, _) => (Decimal) card.Owner.PlayerCombatState.OrbQueue.Orbs.GroupBy<OrbModel, ModelId>((Func<OrbModel, ModelId>) (orb => orb.Id)).Count<IGrouping<ModelId, OrbModel>>()))
      });
    }
  }
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    IEnumerable<CardModel> cardModels = await CardPileCmd.Draw(choiceContext, ((CalculatedVar) this.DynamicVars["CalculatedCards"]).Calculate(cardPlay.Target), this.Owner);
  }
