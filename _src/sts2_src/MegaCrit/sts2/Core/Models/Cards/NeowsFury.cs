...
// Type: MegaCrit.Sts2.Core.Models.Cards.NeowsFury
...
public sealed class NeowsFury : CardModel
{
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    int maxCount = Math.Min(this.DynamicVars.Cards.IntValue, CardPile.MaxCardsInHand - PileType.Hand.GetPile(this.Owner).Cards.Count);
    if (maxCount <= 0)
      return;
    IReadOnlyList<CardPileAddResult> cardPileAddResultList = await CardPileCmd.Add(await CardSelectCmd.FromCombatPile(choiceContext, PileType.Discard.GetPile(this.Owner), this.Owner, new CardSelectorPrefs(this.SelectionScreenPrompt, 0, maxCount)), PileType.Hand);
  }
