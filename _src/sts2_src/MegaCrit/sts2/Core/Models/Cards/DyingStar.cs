...
// Type: MegaCrit.Sts2.Core.Models.Cards.DyingStar
...
public sealed class DyingStar : CardModel
{
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    AttackCommand attackCommand = await DamageCmd.Attack(this.DynamicVars.Damage.BaseValue).FromCard((CardModel) this, cardPlay).TargetingAllOpponents(this.CombatState).WithHitFx("vfx/vfx_starry_impact").SpawningHitVfxOnEachCreature().Execute(choiceContext);
    foreach (Creature enemy in (IEnumerable<Creature>) enemies)
    {
      DyingStarPower dyingStarPower = await PowerCmd.Apply<DyingStarPower>(choiceContext, enemy, this.DynamicVars["StrengthLoss"].BaseValue, this.Owner.Creature, (CardModel) this);
