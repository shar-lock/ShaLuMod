...
// Type: MegaCrit.Sts2.Core.Models.Cards.Volley
...
public sealed class Volley : CardModel
{
  ...
  {
    AttackCommand attackCommand = await DamageCmd.Attack(this.DynamicVars.Damage.BaseValue).WithHitCount(this.ResolveEnergyXValue()).FromCard((CardModel) this, cardPlay).TargetingRandomOpponents(this.CombatState).WithHitFx("vfx/vfx_attack_slash").Execute(choiceContext);
  }
