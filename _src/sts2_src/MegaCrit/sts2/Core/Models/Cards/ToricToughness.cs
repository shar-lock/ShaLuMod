...
// Type: MegaCrit.Sts2.Core.Models.Cards.ToricToughness
...
public sealed class ToricToughness : CardModel
{
  ...
  {
    Decimal blockAmount = await CreatureCmd.GainBlock(this.Owner.Creature, this.DynamicVars.Block, cardPlay);
    (await PowerCmd.Apply<ToricToughnessPower>(choiceContext, this.Owner.Creature, this.DynamicVars["Turns"].BaseValue, this.Owner.Creature, (CardModel) this))?.SetBlock(blockAmount);
  }
