...
// Type: MegaCrit.Sts2.Core.Models.Cards.ByrdSwoop
...
public sealed class ByrdSwoop : CardModel
{
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    AttackCommand attackCommand = await DamageCmd.Attack(this.DynamicVars.Damage.BaseValue).FromCard((CardModel) this, cardPlay).Targeting(cardPlay.Target).WithAttackerAnim("Attack", this.Owner.Character.AttackAnimDelay, this.Owner.PlayerCombatState.GetPet<Byrdpip>()).WithHitFx("vfx/vfx_attack_slash", "event:/sfx/byrdpip/byrdpip_attack").Execute(choiceContext);
  }
