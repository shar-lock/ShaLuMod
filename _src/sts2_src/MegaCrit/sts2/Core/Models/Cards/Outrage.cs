...
// Type: MegaCrit.Sts2.Core.Models.Cards.Outrage
...
public sealed class Outrage : CardModel
{
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    foreach (Creature creature in this.CombatState.GetTeammatesOf(this.Owner.Creature).Where<Creature>((Func<Creature, bool>) (c => c != null && c.IsAlive && c.IsPlayer)))
      CardCmd.PreviewCardPileAdd(await CardPileCmd.AddGeneratedCardToCombat(this.CreateCloneForPlayer(creature.Player), PileType.Discard, this.Owner), 2.2f);
  }
