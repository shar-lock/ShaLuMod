...
// Type: MegaCrit.Sts2.Core.Models.Cards.HelixDrill
    ...
    get
    {
      return (IEnumerable<DynamicVar>) new \u003C\u003Ez__ReadOnlyArray<DynamicVar>(new DynamicVar[4]
      {
        ...
        (DynamicVar) new CalculatedVar("CalculatedHits").WithMultiplier((Func<CardModel, Creature, Decimal>) ((card, _) =>
        {
          if (card.Pile == null)
            return 0M;
          int canonicalVars = CombatManager.Instance.History.Entries.OfType<EnergySpentEntry>().Where<EnergySpentEntry>((Func<EnergySpentEntry, bool>) (e => e.HappenedThisTurn(card.CombatState) && e.Actor.Player == card.Owner)).Sum<EnergySpentEntry>((Func<EnergySpentEntry, int>) (c => c.Amount));
          if (card.Pile.Type == PileType.Play)
            canonicalVars -= card.EnergyCost.GetWithModifiers(CostModifiers.All);
          return (Decimal) canonicalVars;
        }))
      });
    }
  }
