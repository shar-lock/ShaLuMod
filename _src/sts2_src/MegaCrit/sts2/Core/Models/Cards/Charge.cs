...
// Type: MegaCrit.Sts2.Core.Models.Cards.Charge
...
public sealed class Charge : CardModel
{
  ...
  protected override IEnumerable<IHoverTip> ExtraHoverTips
  {
    get
    {
      return (IEnumerable<IHoverTip>) new \u003C\u003Ez__ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.FromCard<MinionDiveBomb>(this.IsUpgraded));
    }
  }
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    {
      CardPileAddResult? nullable = await CardCmd.TransformTo<MinionDiveBomb>(original);
      if (this.IsUpgraded && nullable.HasValue)
        CardCmd.Upgrade(nullable.Value.cardAdded);
    }
