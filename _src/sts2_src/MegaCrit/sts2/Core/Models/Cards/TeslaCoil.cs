...
// Type: MegaCrit.Sts2.Core.Models.Cards.TeslaCoil
...
public sealed class TeslaCoil : CardModel
{
  ...
  protected override IEnumerable<IHoverTip> ExtraHoverTips
  {
    get
    {
      return (IEnumerable<IHoverTip>) new \u003C\u003Ez__ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.FromOrb<LightningOrb>());
    }
  }
  ...
  protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
  {
    ...
    foreach (LightningOrb lightningOrb in this.Owner.PlayerCombatState.OrbQueue.Orbs.OfType<LightningOrb>().ToList<LightningOrb>())
    {
      await OrbCmd.Passive(choiceContext, (OrbModel) lightningOrb, cardPlay.Target);
      if (this.IsUpgraded)
        await OrbCmd.Passive(choiceContext, (OrbModel) lightningOrb, cardPlay.Target);
    }
  }
