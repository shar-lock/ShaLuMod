...
// Type: MegaCrit.Sts2.Core.Models.Cards.Stardust
...
public sealed class Stardust : CardModel
{
  ...
  {
    AttackCommand attackCommand = await DamageCmd.Attack(this.DynamicVars.Damage.BaseValue).WithHitCount(this.ResolveStarXValue()).FromCard((CardModel) this, cardPlay).TargetingRandomOpponents(this.CombatState).WithHitFx("vfx/vfx_starry_impact").Execute(choiceContext);
  }
