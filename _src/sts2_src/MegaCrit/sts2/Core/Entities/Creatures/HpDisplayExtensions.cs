// Decompiled with JetBrains decompiler
// Type: MegaCrit.Sts2.Core.Entities.Creatures.HpDisplayExtensions
// Assembly: sts2, Version=0.1.0.0, Culture=neutral, PublicKeyToken=null
// MVID: DB296A8D-D1DB-4F9A-B229-9E9A5BB4D47E
// Assembly location: D:\shaluMod\ShaLuMod\_src\sts2.dll

#nullable disable
namespace MegaCrit.Sts2.Core.Entities.Creatures;

public static class HpDisplayExtensions
{
  public static bool IsInfinite(this HpDisplay display)
  {
    bool flag;
    switch (display)
    {
      case HpDisplay.InfiniteWithNumbers:
      case HpDisplay.InfiniteWithoutNumbers:
        flag = true;
        break;
      default:
        flag = false;
        break;
    }
    return flag;
  }

  public static bool ShowsNumbers(this HpDisplay display)
  {
    bool flag;
    switch (display)
    {
      case HpDisplay.Normal:
      case HpDisplay.InfiniteWithNumbers:
        flag = true;
        break;
      default:
        flag = false;
        break;
    }
    return flag;
  }
}
