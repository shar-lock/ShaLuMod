// Decompiled with JetBrains decompiler
// Type: MegaCrit.Sts2.Core.Timeline.Epochs.Defect6Epoch
// Assembly: sts2, Version=0.1.0.0, Culture=neutral, PublicKeyToken=null
// MVID: DB296A8D-D1DB-4F9A-B229-9E9A5BB4D47E
// Assembly location: D:\shaluMod\ShaLuMod\_src\sts2.dll

using MegaCrit.Sts2.Core.Models;
using MegaCrit.Sts2.Core.Models.Relics;
using MegaCrit.Sts2.Core.Nodes.Screens.Timeline;
using System;
using System.Collections.Generic;
using System.Runtime.InteropServices;

#nullable enable
namespace MegaCrit.Sts2.Core.Timeline.Epochs;

public class Defect6Epoch : EpochModel
{
  public override string Id => "DEFECT6_EPOCH";

  public override EpochEra Era => EpochEra.Blight2;

  public override int EraPosition => 1;

  public override string StoryId => "Defect";

  public static List<RelicModel> Relics
  {
    get
    {
      int capacity = 3;
      List<RelicModel> relics = new List<RelicModel>(capacity);
      CollectionsMarshal.SetCount<RelicModel>(relics, capacity);
      Span<RelicModel> span = CollectionsMarshal.AsSpan<RelicModel>(relics);
      int num1 = 0;
      span[num1] = (RelicModel) ModelDb.Relic<GoldPlatedCables>();
      int num2 = num1 + 1;
      span[num2] = (RelicModel) ModelDb.Relic<EmotionChip>();
      int num3 = num2 + 1;
      span[num3] = (RelicModel) ModelDb.Relic<PowerCell>();
      return relics;
    }
  }

  public override string UnlockText => this.CreateRelicUnlockText(Defect6Epoch.Relics);

  public override void QueueUnlocks()
  {
    NTimelineScreen.Instance.QueueRelicUnlock(Defect6Epoch.Relics);
  }
}
